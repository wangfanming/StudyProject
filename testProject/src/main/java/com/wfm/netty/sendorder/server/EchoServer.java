            ...
            serverBootstrap.group(eventLoopGroup).channel(NioServerSocketChannel.class).localAddress("localhost", port)
                    .childHandler(new ChannelInitializer<Channel>() {
                        ...
                        protected void initChannel(Channel ch) throws Exception {
                            ch.pipeline().addLast(new EchoServerInHandler1());
                            ch.pipeline().addLast(new EchoServerOutHandler1());
                            ch.pipeline().addLast(new EchoServerOutHandler2());
                            ch.pipeline().addLast(new EchoServerInHandler2());
